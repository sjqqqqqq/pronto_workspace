...
function GateFidelityY(xT)
    Y = [0 -im; im 0]
    UT = [xT[1]+xT[5]im xT[3]+xT[7]im; xT[2]+xT[6]im xT[4]+xT[8]im]
    return (abs(tr(UT*Y'))^2+2)/6
end

## Two-level Qubit model

@kwdef struct FluxY <: PRONTO.Model{9,1}
    kl::Float64
    kr::Float64
    T::Float64 
end

@define_f FluxY begin
    E0 = 0.0
    E1 = 0.65225
    H0 = diagm([E0, E1])
    H00 = kron(I(2),H0)
...
@define_Q FluxY I(9)
@define_R FluxY kr*I(1)
PRONTO.Pf(θ::FluxY,α,μ,tf) = SMatrix{9,9,Float64}(I(9))

resolve_model(FluxY)

## Compute the optimal solution

θ = FluxY(kl=0.03,kr=10,T=100)
τ = t0,tf = 0,θ.T
ψ1 = [1;0]
ψ2 = [0;1]
