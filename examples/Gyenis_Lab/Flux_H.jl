...
function GateFidelityH(xT)
    H = 1/sqrt(2)*[1 1; 1 -1]
    UT = [xT[1]+xT[5]im xT[3]+xT[7]im; xT[2]+xT[6]im xT[4]+xT[8]im]
    return (abs(tr(UT*H'))^2+2)/6
end

## Two-level Qubit model

@kwdef struct FluxH <: PRONTO.Model{9,1}
    kl::Float64
    kr::Float64
    T::Float64 
end

@define_f FluxH begin
    E0 = 0.0
    E1 = 0.65225
    H0 = diagm([E0, E1])
    H00 = kron(I(2),H0)
...
@define_l FluxH begin
    kl/2*u'*I*u + 1/2*max(kl,10*100^(-0.2*t),10*100^(0.2*(t-T)))*x[9]'*I*x[9]
end

@define_m FluxH begin
    ψ1 = [1/sqrt(2);1/sqrt(2)]
    ψ2 = [1/sqrt(2);-1/sqrt(2)]
    xf = vec([ψ1;ψ2;0*ψ1;0*ψ2;0])
    return 1/2*(x-xf)'*I(9)*(x-xf)
end

@define_Q FluxH I(9)
@define_R FluxH kr*I(1)
PRONTO.Pf(θ::FluxH,α,μ,tf) = SMatrix{9,9,Float64}(I(9))

resolve_model(FluxH)

## Compute the optimal solution

θ = FluxH(kl=0.03,kr=10,T=100)
τ = t0,tf = 0,θ.T
ψ1 = [1;0]
ψ2 = [0;1]
