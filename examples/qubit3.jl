...
@kwdef struct Qubit3 <: PRONTO.Model{8,1}
    kl::Float64 # stage cost gain
    kq::Float64 
end

@define_f Qubit3 begin
    ...
    H1 = 2*π*[0 0.1 0.3; 0.1 0 0.5; 0.3 0.5 0]

    Hc = Matrix{ComplexF64}[]
    push!(Hc, H1)
   
    
    L1 = 0.000 * [1,0,0] * [0,0,1]'
    L2 = 0.000 * [0,1,0] * [0,0,1]'
...
@define_l Qubit3 begin
    kl/2*u'*I*u + kq/2*real([0 0 1]*x2rho(x)*[0;0;1])[1]
end

@define_m Qubit3 begin
    xf = psi2x([0;1;0])
...
resolve_model(Qubit3)

PRONTO.Pf(θ::Qubit3,α,μ,tf) = SMatrix{8,8,Float64}(I(8))

## ----------------------------------- compute the optimal solution ----------------------------------- ##

θ = Qubit3(kl=0.001,kq=0.1)
t0,tf = τ = (0,10)
x0 = SVector{8}(psi2x([1;0;0]))
μ = t->SVector{1}(cos(2*π*t))
...
# lines!(ax2, ts, e1; color=:red, linewidth=2)
# lines!(ax2, ts, e2; color=:blue, linewidth=2)
# # lines!(ax2, ts, e3; color=:green, linewidth=2)
